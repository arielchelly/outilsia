    ...
    const tick = (now: number) => {
      ...
      for (let i = 0; i < positions.length; i++) {
        ...
        for (let j = i + 1; j < positions.length; j++) {
          ...
          if (d < CONNECT_DISTANCE && d > 30) {
            ...
            ctx.beginPath();
            ctx.strokeStyle = `rgba(232, 200, 120, ${op.toFixed(3)})`;
      ...
      for (const pulse of pulsesRef.current) {
        ...
        for (let k = 0; k < 4; k++) {
          ...
          ctx.beginPath();
          ctx.fillStyle = `rgba(232, 200, 120, ${(0.85 * trailFade).toFixed(3)})`;
        ...
        grad.addColorStop(0.4, `rgba(232, 200, 120, ${(0.5 * fade).toFixed(3)})`);
    ...
    >
      ...
      {stars.map((s) => (
        <div
          key={s.domain}
          data-star=""
          className="absolute top-0 left-0 rounded-full bg-white border border-white/30 overflow-hidden flex items-center justify-center will-change-transform shadow-[0_4px_18px_rgba(0,0,0,0.5),0_0_0_1px_rgba(232,200,120,0.1)]"
          style={{
            width: LOGO_SIZE,
            height: LOGO_SIZE,
              ...
              if (step === 0) {
                ...
                if (parent) {
                  parent.style.background =
                    'linear-gradient(135deg, #E8C878 0%, #C9A84C 100%)';
                  parent.innerHTML = `<span style="font-family:var(--font-display);color:#080B14;font-weight:600;font-size:1.1rem;">${s.name.charAt(0)}</span>`;
                }
              }
            }}
          />
