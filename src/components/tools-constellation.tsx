...
export function ToolsConstellation() {
  ...
  useEffect(() => {
    ...
    if (typeof IntersectionObserver !== 'undefined') {
      io = new IntersectionObserver(
        (entries) => {
          ...
          // Restart RAF only when transitioning hidden → visible.
          if (visibleRef.current && !wasVisible && rafRef.current === null) {
    ...
    const tick = (now: number) => {
      // Stop scheduling work entirely when off-screen — frees the main thread
      // for other tasks (Lighthouse audits, smooth scrolling on long pages).
